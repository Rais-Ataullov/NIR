...
void rotation(Vec3f& v, const Vec3f& angles) {
    ...
    /*float rotated_x = v.x * (cos(alpha) * cos(gamma) - sin(alpha) * cos(beta) * sin(gamma)) + v.y * (-sin(alpha) * cos(gamma) - cos(alpha) * cos(beta) * sin(gamma)) +
        v.z * (sin(beta) * sin(gamma));
    float rotated_y = v.x * (cos(alpha) * sin(gamma) + sin(alpha) * cos(beta) * cos(gamma)) + v.y * (-sin(alpha) * sin(gamma) + cos(alpha) * cos(beta) * cos(gamma)) +
        v.z * (-sin(beta) * cos(gamma));
    float rotated_z = v.x * sin(alpha) * sin(beta) + v.y * cos(alpha) * sin(beta) + v.z * cos(beta);*/
...
bool scene_intersect(const Vec3f& orig, const Vec3f& dir, const ObjModel& model, Vec3f& hit, Vec3f& N, Material& material) {
    ...
    if (fabs(dir.y) > 1e-3) {
        ...
        if (d > 0 && fabs(pt.x) < 10 && pt.z<-10 && pt.z>-30 && d < object_dist) {
            ...
            hit = pt;
            N = Vec3f(0, 1, 0);
            material.diffuse_color = Vec3f(1, .7, .3) * .3; //(int(.5 * hit.x + 1000) + int(.5 * hit.z)) & 1 ? Vec3f(1, 1, 1) : Vec3f(1, .7, .3);
        }
    }
    return std::min(object_dist, checkerboard_dist) < 10000;
}
...
Vec3f cast_ray(const Vec3f& orig, const Vec3f& dir, const ObjModel& model, const std::vector<Light>& lights, size_t depth = 0) {
    ...
    if (depth > 4 || !scene_intersect(orig, dir, model, point, N, material)) {
...
#pragma omp parallel for
    ...
    //ofs.open("./out_1.ppm", std::ios::binary);
    //ofs << "P6\n" << width << " " << height << "\n255\n";
